...
class GaussianDiffusion:
    ...
    def Modified_p_sample(
        self, model, x_t1, x_t2, t, clip_denoised=True, denoised_fn=None, model_kwargs=None,
        top_p=None, mask=None, x_start=None,
    ):
        out = self.p_mean_variance(
            model,
        ...
        if top_p is not None and top_p > 0:
            noise_t1 = th.randn_like(x_t1)
            noise_t2 = th.randn_like(x_t2)
            replace_mask_t1 = th.abs(noise_t1) > top_p
            replace_mask_t2 = th.abs(noise_t2) > top_p
            
            while replace_mask_t1.any():
                noise_t1[replace_mask_t1] = th.randn_like(noise_t1[replace_mask_t1])
                replace_mask_t1 = th.abs(noise_t1) > top_p
            
            while replace_mask_t2.any():
                noise_t2[replace_mask_t2] = th.randn_like(noise_t2[replace_mask_t2])
                replace_mask_t2 = th.abs(noise_t2) > top_p
            
            assert (th.abs(noise_t1) <= top_p).all()
            assert (th.abs(noise_t2) <= top_p).all()
        else:
            noise_t1 = th.randn_like(x_t1)
            noise_t2 = th.randn_like(x_t2)
        
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x_t1.shape) - 1)))
        )  # no noise when t == 0
        
        sample = (
            out["mean"] + 
            nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise_t1 +
            nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise_t2
        )
